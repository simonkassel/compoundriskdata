...
altflag$EMERGING_RISK_COVID_RESPONSE_CAPACITY_AV <- geometricmeanRow(altflag[c(
...
altflag$EMERGING_RISK_MACROECONOMIC_EXPOSURE_TO_COVID_AV <- geometricmeanRow(altflag[c(
...
altflag$EMERGING_RISK_FRAGILITY_INSTITUTIONS_AV <- ifelse(is.na(altflag$NH_INFORM_CRISIS_Type) | altflag$NH_INFORM_CRISIS_Type != "Complex crisis", geometricmeanRow(altflag[c(
...
altflag$EMERGING_RISK_COVID_RESPONSE_CAPACITY_SQ_ALT <- geometricmeanRow(cbind.data.frame(
  altflag$H_Oxrollback_score_norm_plus1,
...
reliabilitysheet <- globalrisk %>%
  mutate(
    ...
    RELIABILITY_EXISTING_FRAGILITY_INSTITUTIONS = rowSums(is.na(globalrisk %>%
      select(
        Fr_INFORM_Fragility_Score_norm,
        Fr_FSI_Score_norm,
        Fr_ACLED_event_same_month_difference_perc_norm,
        Fr_ACLED_fatal_same_month_difference_perc_norm
      )),
...
# Write alternative flag sheet to csv
write.csv(alt, 'data/processed/alternative_flag_sheet.csv', row.names = FALSE)
